...
<body>
  ...
  <div class="container">
        ...
        <div class="modal-content">
          ...
          <div class="modal-filter">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-success mt-3" id="applyFilter">Apply Filter</button>
              <button type="button" class="btn btn-secondary mt-3" id="cancelFilter" data-bs-dismiss="modal">Cancel</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <!--Condition to Only Allow One Table Result at a time-->
    <script>
      // Get the filter result container and result section elements
      var filterResultContainer = document.getElementById('filterResultContainer');
      var resultSection = document.getElementById('resultSection');

      function checkTables() {
        // Check if there is a table in the filter result container
        var filterTable = filterResultContainer.getElementsByTagName('table')[0];

        // Check if there is a table in the result section
        var resultTable = resultSection.getElementsByTagName('table')[0];

        // If there is a table in the filter result container, show it and hide the result section
        if (filterTable) {
          filterResultContainer.style.display = 'block';
          resultSection.style.display = 'none';
        }
        // If there is a table in the result section, show it and hide the filter result container
        else if (resultTable) {
          resultSection.style.display = 'block';
          filterResultContainer.style.display = 'none';
        }
      }
